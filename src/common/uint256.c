...
void readu256LE(const uint8_t *const buffer, uint256_t *const target) {
...
void mul256(const uint256_t *const number1,
    ...
    cx_math_mult((uint8_t *)result, num1, num2, sizeof(num1));
    for (uint8_t i = 0; i < 4; i++) {
...
bool tostring256_signed(const uint256_t *const number,
    ...
    if (base == 10) {
        memset(&one_val, 0, sizeof(one_val));
        LOWER(LOWER(one_val)) = 1;
        memset(&two_val, 0, sizeof(two_val));
