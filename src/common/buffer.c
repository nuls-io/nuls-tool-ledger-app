...
uint8_t G_malloc_buff[MALLOC_BUFF_MAX_LEN];


void buffer_init()
{
	g_buffer.ptr = (uint8_t *)G_malloc_buff;

	g_buffer.size= MALLOC_BUFF_MAX_LEN;

	g_buffer.offset = 0;

}

 uint8_t * buffer_malloc(size_t n) {
 	 uint8_t *ptr;

	
	ptr = (uint8_t *)(g_buffer.ptr + g_buffer.offset);

	if(!buffer_seek_cur(&g_buffer, n)) {
		return NULL;
	}
...
bool buffer_move(buffer_t *buffer, uint8_t *out, size_t out_len) {
    ...
    buffer_seek_cur(buffer, buffer->size - buffer->offset);
...
bool buffer_spc_move(buffer_t *buffer, uint8_t *out, size_t out_len) {
    if (buffer->size - buffer->offset <  out_len) {
        return false;
    }
