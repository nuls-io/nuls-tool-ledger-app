/*****************************************************************************
 *   Ledger App Boilerplate.
 *   (c) 2020 Ledger SAS.
 *
 *  Licensed under the Apache License, Version 2.0 (the "License");
 *  you may not use this file except in compliance with the License.
 *  You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 *  Unless required by applicable law or agreed to in writing, software
 *  distributed under the License is distributed on an "AS IS" BASIS,
 *  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 *  See the License for the specific language governing permissions and
 *  limitations under the License.
 *****************************************************************************/

#include <stdint.h>   // uint*_t
#include <stdbool.h>  // bool
#include <stddef.h>   // size_t
#include <string.h>   // memset, explicit_bzero

#include "os.h"
#include "cx.h"

#include "address.h"
#include "get_public_key.h"
#include "../globals.h"
#include "../types.h"
#include "../io.h"
#include "../sw.h"
#include "../crypto.h"
#include "../common/buffer.h"
#include "../ui/display.h"
#include "../helper/send_response.h"



bool init_public_key(buffer_t *cdata, bool is_gen_address) {

	cx_ecfp_private_key_t private_key = {0};
	cx_ecfp_public_key_t public_key = {0};


	PRINTF("************start init_public_key*****************\n");
	//AddressType
	G_context.pk_info.type = 0x01;

	//chainId 
	G_context.pk_info.chain_id = 0x01;


	if (!buffer_read_u8(cdata, &G_context.bip32_path_len) || !buffer_read_bip32_path(cdata, G_context.bip32_path, (size_t) G_context.bip32_path_len)) {
		io_send_sw(SW_WRONG_DATA_LENGTH);
		goto __err;
	}


	PRINTF("input.bip32_path_len=%d\n", G_context.bip32_path_len);

	if(!is_gen_address) {
		return true;
	}

	// derive private key according to BIP32 path
	int error = crypto_derive_private_key(&private_key,
										G_context.pk_info.chain_code,
										G_context.bip32_path,
										G_context.bip32_path_len);
	if (error != 0) {
	  io_send_sw(error);
	  goto __err;
	}


    BEGIN_TRY {
        TRY {

			// generate corresponding public key
			crypto_init_public_key(&private_key, &public_key, G_context.pk_info.raw_public_key);

			crypto_compress_public_key(G_context.pk_info.raw_public_key, G_context.pk_info.compressed_public_key);
	     }
        CATCH_OTHER(e) {
            error = e;
        }
        FINALLY {
            explicit_bzero(&private_key, sizeof(private_key));
        }
    }
    END_TRY;	  


	address_from_pubkey(G_context.pk_info.compressed_public_key, G_context.pk_info.chain_id, 
	   G_context.pk_info.type,	G_context.pk_info.address, G_context.pk_info.address_base_58);

	PRINTF("ouput.address_base_58=%s\n",  G_context.pk_info.address_base_58);


	PRINTF("*****************end init_public_key*************\n");

	return true;

	
__err:
	return false;
	
}

int handler_get_public_key(buffer_t *cdata, bool display) {
	explicit_bzero(&G_context, sizeof(G_context));
	G_context.req_type = CONFIRM_ADDRESS;
	G_context.state = STATE_NONE;


	if(!init_public_key(cdata, true)) {
		return -1;
	}
		
	if (display) {
		 return ui_display_public_key();
	} else {
		return helper_send_response_pubkey();
	}



}
